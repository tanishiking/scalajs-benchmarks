...
class Plane(position: Vector, val d: Float, material: Material) extends Shape(position, material) {
  ...
  def intersect(ray: Ray): IntersectionInfo = {
    ...
    val color = if (this.material.hasTexture) {
      ...
      val u: Float = intersection.dot(vU)
      val v: Float = intersection.dot(vV)
...
class Sphere(position: Vector, radius: Float, material: Material) extends Shape(position, material) {
  ...
  def intersect(ray: Ray): IntersectionInfo = {
    ...
    val distance = (-B) - math.sqrt(D).toFloat
