...
class Color(val red: Float, val green: Float, val blue: Float) {
  ...
  def trim(light: Float): Float =
    if (light <= 0.0f) 0.0f
    else if (light > 1.0f) 1.0f
    else light
  ...
  def addScalar(scalar: Float): Color =
    new Color(trim(red + scalar), trim(green + scalar), trim(blue + scalar))

  def *(that: Color): Color =
    new Color(red * that.red, green * that.green, blue * that.blue)

  def multiplyScalar(f: Float): Color =
    new Color(red * f, green * f, blue * f)

  def blend(that: Color, w: Float): Color =
    this.multiplyScalar(1.0f - w) + that.multiplyScalar(w)
