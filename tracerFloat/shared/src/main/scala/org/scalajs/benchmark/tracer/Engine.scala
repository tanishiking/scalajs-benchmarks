...
class Engine(val config: EngineConfiguration) {
  ...
  def renderScene(scene: Scene, canvasContext: CanvasRenderingContext2D): Unit = {
    for {
      ...
      val yp = y * 1.0f / config.canvasHeight * 2 - 1;
      val xp = x * 1.0f / config.canvasWidth * 2 - 1;
    ...
    for (light <- scene.lights) {
      ...
      if (config.renderShadows) {
        ...
        if (shadowInfo.isHit && shadowInfo.shape != info.shape) {
          val vA = color.multiplyScalar(0.5f)
          val dB = (0.5f * math.pow(shadowInfo.shape.material.transparency, 0.5f).toFloat)
      ...
      if (config.renderHighlights && !shadowInfo.isHit && info.shape.material.gloss > 0) {
        ...
        var glossWeight = math.pow(math.max(info.normal.dot(H), 0), shininess).toFloat
