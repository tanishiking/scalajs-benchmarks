...
class Camera(val position: Vector, val lookAt: Vector, val up: Vector) {
  ...
  val REVERSE_Y_VECTOR = new Vector(1.0f, -1.0f, 1.0f)

  def getRay(vx: Float, vy: Float): Ray = {
...
class Background(val color: Color, val ambience: Float)

class Light(val position: Vector, val color: Color, val intensity: Float = 10.0f)
