 ...
 *
 * This is a WRONG version that uses Ints everywhere instead of Longs. It
 * computes the wrong hashes. It is only useful to fairly compare Ints vs
 * Longs.
 */
...
object SHA512Context {
   ...
   */
  @inline
  def getUInt64BE(b: Array[Byte], i: Int): Int = {
    ((b(i) & 0xff) << 56) |
    ((b(i + 1) & 0xff) << 48) |
    ((b(i + 2) & 0xff) << 40) |
    ((b(i + 3) & 0xff) << 32) |
  ...
  @inline
  def putUInt64BE(b: Array[Byte], i: Int, n: Int): Unit = {
    b(i) = (n >> 56).toByte
    b(i + 1) = (n >> 48).toByte
    b(i + 2) = (n >> 40).toByte
    b(i + 3) = (n >> 32).toByte
...
class SHA512Context(val is384: Boolean) {
  ...
  val total = new Array[Int](2) // number of bytes processed
  val state = new Array[Int](8) // intermediate digest state
  ...
  def init(): Unit = {
    ...
    if (!is384) {
      ...
      state(1) = 0xBB67AE8584CAA73BL.toInt
      state(2) = 0x3C6EF372FE94F82BL.toInt
      state(3) = 0xA54FF53A5F1D36F1L.toInt
      state(4) = 0x510E527FADE682D1L.toInt
      state(5) = 0x9B05688C2B3E6C1FL.toInt
      state(6) = 0x1F83D9ABFB41BD6BL.toInt
      state(7) = 0x5BE0CD19137E2179L.toInt
    } else {
      // SHA-384
      state(0) = 0xCBBB9D5DC1059ED8L.toInt
      state(1) = 0x629A292A367CD507L.toInt
      state(2) = 0x9159015A3070DD17L.toInt
      state(3) = 0x152FECD8F70E5939L.toInt
      state(4) = 0x67332667FFC00B31L.toInt
      state(5) = 0x8EB44A8768581511L.toInt
      state(6) = 0xDB0C2E0D64F98FA7L.toInt
      state(7) = 0x47B5481DBEFA4FA4L.toInt
    }
  }

  def process(data: Array[Byte], start: Int): Unit = {
    import java.lang.Integer.{rotateRight => rotr}

    @inline def shr(x: Int, distance: Int): Int = x >>> distance

    @inline def S0(x: Int): Int = rotr(x, 1) ^ rotr(x, 8) ^ shr(x, 7)
    @inline def S1(x: Int): Int = rotr(x, 19) ^ rotr(x, 61) ^ shr(x, 6)

    @inline def S2(x: Int): Int = rotr(x, 28) ^ rotr(x, 34) ^ rotr(x, 39)
    @inline def S3(x: Int): Int = rotr(x, 14) ^ rotr(x, 18) ^ rotr(x, 41)

    @inline def F0(x: Int, y: Int, z: Int): Int = ((x & y) | (z & (x | y)))
    @inline def F1(x: Int, y: Int, z: Int): Int = (z ^ (x & (y ^ z)))

    // Returns (newD, newH) - the C implementation updates them in-place
    @inline def P(a: Int, b: Int, c: Int, d: Int, e: Int, f: Int, g: Int,
        h: Int, x: Int, K: Int): (Int, Int) = {
